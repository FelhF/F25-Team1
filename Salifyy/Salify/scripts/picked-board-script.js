...
fetch(`http://localhost:8080/boards/${boardId}/deals`)                  // Fetch deals in the board
    .then(res => res.json())
    .then(deals => {
        console.log(deals); 
...
function createDealElement(deal) {                                  //Create deal card
    ...
    const removeFromBoardBtn = dealCard.querySelector('.RemoveFromBoardBtn');               // REMOVE DEAL FROM BOARD
